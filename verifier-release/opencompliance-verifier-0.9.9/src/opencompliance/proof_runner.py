...
def _remote_access_posture_block(name: str, payload: dict) -> str:
    return f"""
def {name} : OpenCompliance.Controls.RemoteAccessPostureEvidence := {{
...
theorem {name}_proved :
    OpenCompliance.Controls.RemoteAccessPostureDeclared {name} := by
  exact OpenCompliance.Controls.remoteAccessPostureDeclared_of_components {name} rfl rfl (by decide)
"""
...
def _ai_content_provenance_block(name: str, payload: dict) -> str:
    return f"""
def {name} : OpenCompliance.Controls.AiContentProvenanceEvidence := {{
...
theorem {name}_proved :
    OpenCompliance.Controls.AiContentProvenanceConfigured {name} := by
  exact OpenCompliance.Controls.aiContentProvenanceConfigured_of_flags {name} rfl rfl rfl
"""


def _ai_task_envelope_block(name: str, payload: dict) -> str:
    return f"""
def {name} : OpenCompliance.Controls.AiTaskEnvelopeEvidence := {{
...
theorem {name}_proved :
    OpenCompliance.Controls.AiTaskEnvelopeBounded {name} := by
  exact OpenCompliance.Controls.aiTaskEnvelopeBounded_of_flags {name} rfl rfl rfl rfl rfl
"""


def _ai_rights_ready_run_trace_block(name: str, payload: dict) -> str:
    return f"""
def {name} : OpenCompliance.Controls.AiRightsReadyRunTraceEvidence := {{
...
theorem {name}_proved :
    OpenCompliance.Controls.AiRightsReadyRunTraceRetained {name} := by
  exact OpenCompliance.Controls.aiRightsReadyRunTraceRetained_of_flags {name} rfl rfl rfl rfl rfl
"""


def _privacy_notice_publication_block(name: str, payload: dict) -> str:
    return f"""
def {name} : OpenCompliance.Controls.PrivacyNoticePublicationEvidence := {{
  noticePublished := {_bool_literal(payload['noticePublished'])}
  categoriesAndPurposesListed := {_bool_literal(payload['categoriesAndPurposesListed'])}
  rightsProcessLinked := {_bool_literal(payload['rightsProcessLinked'])}
  noticeUrlReachable := {_bool_literal(payload['noticeUrlReachable'])}
  reviewWindowDays := {payload['reviewWindowDays']}
}}

theorem {name}_proved :
    OpenCompliance.Controls.PrivacyNoticePublicationStateCaptured {name} := by
  exact OpenCompliance.Controls.privacyNoticePublicationStateCaptured_of_components {name} rfl rfl rfl rfl (by decide)
"""
...
BLOCK_BUILDERS = {
    ...
    "oc.remote-01": _remote_access_posture_block,
    "oc.ai-05": _ai_content_labeling_block,
    "oc.ai-06": _ai_content_provenance_block,
    "oc.ai-11": _ai_task_envelope_block,
    "oc.ai-12": _ai_rights_ready_run_trace_block,
    "oc.privacy-06": _privacy_notice_publication_block,
}
