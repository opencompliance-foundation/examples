...
LEGALLEAN_RUNTIME_TIMEOUT_SECONDS = int(os.environ.get("OPENCOMPLIANCE_LEAN_RUNTIME_TIMEOUT_SECONDS", "300"))
SUPPORTED_FIXTURES = {
    "minimal",
    "failed",
    "stale",
...
def _minimal_runtime_script(
    ...
    training_signed = training_claim is not None and is_attestation_actor(training_claim.actor_type)
...
def _run_minimal_runtime_decisions(
    ...
    with tempfile.TemporaryDirectory(dir=lean_root) as temp_dir_raw:
        ...
        result = subprocess.run(
            ...
            text=True,
            check=False,
            timeout=LEGALLEAN_RUNTIME_TIMEOUT_SECONDS,
        )
...
def _formal_remote_access_posture_block(base_name: str, payload: dict) -> tuple[tuple[str, ...], str]:
    return (
        ("OpenCompliance.Controls.Network",),
        f"""
def {base_name}_evidence : OpenCompliance.Controls.RemoteAccessPostureEvidence := {{
  remotePathDeclared := {_bool_literal(payload['remotePathDeclared'])}
  devicePostureBounded := {_bool_literal(payload['devicePostureBounded'])}
  systemCount := {payload['systemCount']}
}}

def {base_name}_satisfied : Bool :=
  {base_name}_evidence.remotePathDeclared &&
  {base_name}_evidence.devicePostureBounded &&
  decide (0 < {base_name}_evidence.systemCount)
""",
    )
...
def _formal_ai_content_provenance_block(base_name: str, payload: dict) -> tuple[tuple[str, ...], str]:
    return (
        ("OpenCompliance.Controls.AI",),
        f"""
def {base_name}_evidence : OpenCompliance.Controls.AiContentProvenanceEvidence := {{
  provenanceMetadataEnabled := {_bool_literal(payload['provenanceMetadataEnabled'])}
  originHistoryRetained := {_bool_literal(payload['originHistoryRetained'])}
  verificationChecksPassing := {_bool_literal(payload['verificationChecksPassing'])}
}}

def {base_name}_satisfied : Bool :=
  {base_name}_evidence.provenanceMetadataEnabled &&
  {base_name}_evidence.originHistoryRetained &&
  {base_name}_evidence.verificationChecksPassing
""",
    )


def _formal_ai_task_envelope_block(base_name: str, payload: dict) -> tuple[tuple[str, ...], str]:
    return (
        ("OpenCompliance.Controls.AI",),
        f"""
def {base_name}_evidence : OpenCompliance.Controls.AiTaskEnvelopeEvidence := {{
  purposeDeclared := {_bool_literal(payload['purposeDeclared'])}
  allowedToolClassesDeclared := {_bool_literal(payload['allowedToolClassesDeclared'])}
  allowedDataClassesDeclared := {_bool_literal(payload['allowedDataClassesDeclared'])}
  retentionClassDeclared := {_bool_literal(payload['retentionClassDeclared'])}
  humanReviewLinkagePresent := {_bool_literal(payload['humanReviewLinkagePresent'])}
}}

def {base_name}_satisfied : Bool :=
  {base_name}_evidence.purposeDeclared &&
  {base_name}_evidence.allowedToolClassesDeclared &&
  {base_name}_evidence.allowedDataClassesDeclared &&
  {base_name}_evidence.retentionClassDeclared &&
  {base_name}_evidence.humanReviewLinkagePresent
""",
    )


def _formal_ai_rights_ready_run_trace_block(base_name: str, payload: dict) -> tuple[tuple[str, ...], str]:
    return (
        ("OpenCompliance.Controls.AI",),
        f"""
def {base_name}_evidence : OpenCompliance.Controls.AiRightsReadyRunTraceEvidence := {{
  traceRetentionEnabled := {_bool_literal(payload['traceRetentionEnabled'])}
  requestPurposeCaptured := {_bool_literal(payload['requestPurposeCaptured'])}
  inputClassificationsCaptured := {_bool_literal(payload['inputClassificationsCaptured'])}
  outputReferencesCaptured := {_bool_literal(payload['outputReferencesCaptured'])}
  humanReviewLinkagePresent := {_bool_literal(payload['humanReviewLinkagePresent'])}
}}

def {base_name}_satisfied : Bool :=
  {base_name}_evidence.traceRetentionEnabled &&
  {base_name}_evidence.requestPurposeCaptured &&
  {base_name}_evidence.inputClassificationsCaptured &&
  {base_name}_evidence.outputReferencesCaptured &&
  {base_name}_evidence.humanReviewLinkagePresent
""",
    )


def _formal_privacy_notice_publication_block(base_name: str, payload: dict) -> tuple[tuple[str, ...], str]:
    return (
        (),
        f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(payload['noticePublished'])} &&
  {_bool_literal(payload['categoriesAndPurposesListed'])} &&
  {_bool_literal(payload['rightsProcessLinked'])} &&
  {_bool_literal(payload['noticeUrlReachable'])} &&
  decide (0 < {int(payload['reviewWindowDays'])})
""",
    )
...
FORMAL_BUILDERS = {
    ...
    "oc.remote-01": _formal_remote_access_posture_block,
    "oc.ai-05": _formal_ai_content_labeling_block,
    "oc.ai-06": _formal_ai_content_provenance_block,
    "oc.ai-11": _formal_ai_task_envelope_block,
    "oc.ai-12": _formal_ai_rights_ready_run_trace_block,
    "oc.privacy-06": _formal_privacy_notice_publication_block,
}
...
def {base_name}_satisfied : Bool :=
  ...
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""
...
def _documentary_data_retention_schedule_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['scheduleDefined'])} &&
  {_bool_literal(claim.payload['exceptionHandlingDefined'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_security_commitment_terms_block(base_name: str, claim) -> str:
    agreements_covered = claim.payload["agreementTypesCovered"] > 0
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['termsPresent'])} &&
  {_bool_literal(agreements_covered)} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_security_commitment_governance_block(base_name: str, claim) -> str:
    owner_declared = bool(claim.payload["ownerRole"])
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(owner_declared)} &&
...
def _documentary_outsourced_development_requirement_block(base_name: str, claim) -> str:
    relationships_covered = claim.payload["relationshipScopeCount"] > 0
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['requirementsDefined'])} &&
  {_bool_literal(relationships_covered)} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_outsourced_development_review_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['oversightReviewPerformed'])} &&
  {_bool_literal(claim.payload['exceptionHandlingDefined'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_security_concern_intake_block(base_name: str, claim) -> str:
    owner_declared = bool(claim.payload["ownerRole"])
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['intakePathDefined'])} &&
  {_bool_literal(owner_declared)} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_security_concern_resolution_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['resolutionTrackingCurrent'])} &&
  {_bool_literal(claim.payload['ownersAssigned'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_remote_working_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['procedureCurrent'])} &&
  {_bool_literal(claim.payload['incidentReportingCovered'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_corrective_action_process_block(base_name: str, claim) -> str:
    owner_declared = bool(claim.payload["ownerRole"])
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['processDefined'])} &&
  {_bool_literal(owner_declared)} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_corrective_action_followup_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['followupCurrent'])} &&
  {_bool_literal(claim.payload['ownersAssigned'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_compliance_requirement_register_block(base_name: str, claim) -> str:
    requirement_classes_covered = claim.payload["requirementClassesCovered"] > 0
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['registerCurrent'])} &&
  {_bool_literal(requirement_classes_covered)} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_compliance_requirement_review_block(base_name: str, claim) -> str:
    owner_declared = bool(claim.payload["ownerRole"])
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['reviewCurrent'])} &&
  {_bool_literal(owner_declared)} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_isms_stakeholder_register_block(base_name: str, claim) -> str:
    stakeholder_groups_covered = claim.payload["stakeholderGroupsCovered"] > 0
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['registerCurrent'])} &&
  {_bool_literal(stakeholder_groups_covered)} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_isms_stakeholder_obligation_block(base_name: str, claim) -> str:
    owner_declared = bool(claim.payload["ownerRole"])
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['obligationsCurrent'])} &&
  {_bool_literal(owner_declared)} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_project_security_requirement_block(base_name: str, claim) -> str:
    project_types_covered = claim.payload["projectTypesCovered"] > 0
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['requirementsDefined'])} &&
  {_bool_literal(project_types_covered)} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_project_security_review_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['reviewPerformed'])} &&
  {_bool_literal(claim.payload['exceptionsTracked'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_customer_support_channel_block(base_name: str, claim) -> str:
    channels_documented = claim.payload["channelsDocumented"] > 0
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(channels_documented)} &&
  {_bool_literal(claim.payload['channelsReachable'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_customer_support_governance_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['escalationOwnershipDefined'])} &&
  {_bool_literal(claim.payload['staffingModelCurrent'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_intellectual_property_policy_block(base_name: str, claim) -> str:
    obligation_classes_covered = claim.payload["obligationClassesCovered"] > 0
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['policyCurrent'])} &&
  {_bool_literal(obligation_classes_covered)} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_intellectual_property_issue_block(base_name: str, claim) -> str:
    owner_declared = bool(claim.payload["ownerRole"])
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['handlingRulesCurrent'])} &&
  {_bool_literal(owner_declared)} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_code_of_conduct_block(base_name: str, claim) -> str:
    population_scope_valid = claim.payload["populationScope"] in {"employees", "contractors", "mixed"}
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['acknowledgementsCurrent'])} &&
  {_bool_literal(population_scope_valid)} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_conduct_governance_block(base_name: str, claim) -> str:
    population_scope_valid = claim.payload["populationScope"] in {"employees", "contractors", "mixed"}
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['disciplinaryPathDefined'])} &&
  {_bool_literal(population_scope_valid)} &&
  {_bool_literal(claim.payload['refreshTriggersDefined'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_disciplinary_policy_block(base_name: str, claim) -> str:
    populations_covered = claim.payload["populationsCovered"] > 0
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['policyCurrent'])} &&
  {_bool_literal(populations_covered)} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_disciplinary_action_block(base_name: str, claim) -> str:
    reviewed_breaches_count_valid = claim.payload["reviewedBreachesCount"] >= 0
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['actionRecordsCurrent'])} &&
  {_bool_literal(reviewed_breaches_count_valid)} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_internal_audit_program_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['programCurrent'])} &&
  {_bool_literal(claim.payload['scheduleDefined'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_internal_audit_execution_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['findingsTracked'])} &&
  {_bool_literal(claim.payload['followUpCurrent'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_clear_desk_screen_policy_block(base_name: str, claim) -> str:
    workspace_types_covered = claim.payload["workspaceTypesCovered"] > 0
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['policyCurrent'])} &&
  {_bool_literal(workspace_types_covered)} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_clear_desk_screen_coverage_block(base_name: str, claim) -> str:
    areas_covered = claim.payload["areasCovered"] > 0
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(areas_covered)} &&
  {_bool_literal(claim.payload['communicationCurrent'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_security_community_membership_block(base_name: str, claim) -> str:
    groups_covered = claim.payload["groupsCovered"] > 0
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(groups_covered)} &&
  {_bool_literal(claim.payload['participationCurrent'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_security_community_output_review_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['disseminationPathDefined'])} &&
  {_bool_literal(claim.payload['ownerDefined'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_lawful_basis_register_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['registerMaintained'])} &&
  {_bool_literal(claim.payload['purposeBoundariesDocumented'])} &&
  {_bool_literal(claim.payload['lawfulBasisAssigned'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_privacy_notice_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['noticePublished'])} &&
  {_bool_literal(claim.payload['categoriesAndPurposesListed'])} &&
  {_bool_literal(claim.payload['rightsProcessLinked'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_privacy_impact_assessment_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['assessmentCurrent'])} &&
  {_bool_literal(claim.payload['scopeDocumented'])} &&
  {_bool_literal(claim.payload['signoffAssigned'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""
...
def _documentary_ai_human_review_trigger_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['highImpactTriggerCriteriaDocumented'])} &&
  {_bool_literal(claim.payload['humanReviewOwnerAssigned'])} &&
  {_bool_literal(claim.payload['contestPathDocumented'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""
...
def _documentary_ai_evaluation_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['evaluationScopeDocumented'])} &&
  {_bool_literal(claim.payload['testingLayersDocumented'])} &&
  {_bool_literal(claim.payload['findingsCaptured'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_ai_data_quality_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['dataSourcesEnumerated'])} &&
  {_bool_literal(claim.payload['qualityCriteriaDocumented'])} &&
  {_bool_literal(claim.payload['lineageDocumented'])} &&
  {_bool_literal(claim.payload['stewardAssigned'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""


def _documentary_ai_supplier_transfer_block(base_name: str, claim) -> str:
    return f"""
def {base_name}_satisfied : Bool :=
  {_bool_literal(claim.payload['supplierRegisterCurrent'])} &&
  {_bool_literal(claim.payload['subprocessorRolesDeclared'])} &&
  {_bool_literal(claim.payload['transferMechanismDeclared'])} &&
  {_bool_literal(is_attestation_actor(claim.actor_type))}
"""
...
DOCUMENTARY_BUILDERS = {
    ...
    "data_retention_schedule_attestation": _documentary_data_retention_schedule_block,
    "security_commitment_terms_attestation": _documentary_security_commitment_terms_block,
    "security_commitment_governance_attestation": _documentary_security_commitment_governance_block,
    "outsourced_development_requirement_attestation": _documentary_outsourced_development_requirement_block,
    "outsourced_development_review_attestation": _documentary_outsourced_development_review_block,
    "security_concern_intake_attestation": _documentary_security_concern_intake_block,
    "security_concern_resolution_attestation": _documentary_security_concern_resolution_block,
    "remote_working_attestation": _documentary_remote_working_block,
    "corrective_action_process_attestation": _documentary_corrective_action_process_block,
    "corrective_action_followup_attestation": _documentary_corrective_action_followup_block,
    "compliance_requirement_register_attestation": _documentary_compliance_requirement_register_block,
    "compliance_requirement_review_attestation": _documentary_compliance_requirement_review_block,
    "isms_stakeholder_register_attestation": _documentary_isms_stakeholder_register_block,
    "isms_stakeholder_obligation_attestation": _documentary_isms_stakeholder_obligation_block,
    "project_security_requirement_attestation": _documentary_project_security_requirement_block,
    "project_security_review_attestation": _documentary_project_security_review_block,
    "customer_support_channel_attestation": _documentary_customer_support_channel_block,
    "customer_support_governance_attestation": _documentary_customer_support_governance_block,
    "intellectual_property_policy_attestation": _documentary_intellectual_property_policy_block,
    "intellectual_property_issue_attestation": _documentary_intellectual_property_issue_block,
    "code_of_conduct_attestation": _documentary_code_of_conduct_block,
    "conduct_governance_attestation": _documentary_conduct_governance_block,
    "disciplinary_policy_attestation": _documentary_disciplinary_policy_block,
    "disciplinary_action_attestation": _documentary_disciplinary_action_block,
    "internal_audit_program_attestation": _documentary_internal_audit_program_block,
    "internal_audit_execution_attestation": _documentary_internal_audit_execution_block,
    "clear_desk_screen_policy_attestation": _documentary_clear_desk_screen_policy_block,
    "clear_desk_screen_coverage_attestation": _documentary_clear_desk_screen_coverage_block,
    "security_community_membership_attestation": _documentary_security_community_membership_block,
    "security_community_output_review_attestation": _documentary_security_community_output_review_block,
    "lawful_basis_register_attestation": _documentary_lawful_basis_register_block,
    "privacy_notice_attestation": _documentary_privacy_notice_block,
    "privacy_impact_assessment_attestation": _documentary_privacy_impact_assessment_block,
    ...
    "ai_human_review_trigger_attestation": _documentary_ai_human_review_trigger_block,
    "ai_monitoring_attestation": _documentary_ai_monitoring_block,
    "ai_evaluation_attestation": _documentary_ai_evaluation_block,
    "ai_data_quality_attestation": _documentary_ai_data_quality_block,
    "ai_supplier_transfer_attestation": _documentary_ai_supplier_transfer_block,
}
