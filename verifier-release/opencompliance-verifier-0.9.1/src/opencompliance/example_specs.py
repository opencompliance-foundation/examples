...
def _training_attestation(claim) -> bool:
    return claim.payload["trainingComplete"] is True and claim.actor_type == "human"


def _restore_attestation(claim) -> bool:
    return claim.payload["restoreTestCompleted"] is True and claim.actor_type == "human"
...
def _vendor_attestation(claim) -> bool:
    return (
        ...
        and claim.actor_type == "human"
    )


def _dsr_attestation(claim) -> bool:
    return claim.payload["runbookApproved"] is True and claim.actor_type == "human"
...
AI_GOVERNANCE_FIXTURE = FixtureSpec(
    ...
    verifier_version=PUBLIC_VERIFIER_VERSION,
    bundle_scope_note="Synthetic public-safe ExampleCo proof bundle for a narrow AI governance corridor spanning documentary governance evidence and one machine-checkable disclosure control.",
    next_actions=[
        ...
        "Add typed model-card, evaluation, and post-deployment telemetry exports so more of the AI corridor can move from attestations to machine evidence.",
