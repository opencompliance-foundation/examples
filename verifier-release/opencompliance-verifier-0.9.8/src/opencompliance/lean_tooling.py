...
LAKE_CACHE_IGNORE = shutil.ignore_patterns(".git")


def _required_build_artifacts(lean_root: Path) -> tuple[Path, ...]:
    return (
        lean_root / ".lake" / "build" / "lib" / "lean" / "OpenCompliance" / "Controls" / "Identity.olean",
        lean_root
        / ".lake"
        / "build"
        / "lib"
...
def _build_artifacts_ready(lean_root: Path) -> bool:
    return all(path.exists() for path in _required_build_artifacts(lean_root))
...
def _restore_cached_lake(lean_root: Path) -> bool:
    ...
    if not _build_artifacts_ready(cache_root) or not cache_lake_dir.exists():
        return False
    try:
        shutil.copytree(cache_lake_dir, lean_root / ".lake", dirs_exist_ok=True, ignore=LAKE_CACHE_IGNORE)
    except (FileNotFoundError, shutil.Error):
        shutil.rmtree(lean_root / ".lake", ignore_errors=True)
        shutil.rmtree(cache_root, ignore_errors=True)
        return False
    return _build_artifacts_ready(lean_root)
...
def _refresh_cached_lake(lean_root: Path) -> None:
    ...
    if not _build_artifacts_ready(lean_root) or not source_lake_dir.exists():
        return
    ...
    shutil.copytree(source_lake_dir, temp_root / ".lake", dirs_exist_ok=True, ignore=LAKE_CACHE_IGNORE)
...
def ensure_lean_package_ready(lean_root_raw: str) -> None:
    ...
    if _build_artifacts_ready(lean_root):
        _refresh_cached_lake(lean_root)
        return
    restored_from_cache = _restore_cached_lake(lean_root)
    ...
    if result.returncode != 0:
        raise ValueError(
            ...
            f"ROOT: {lean_root}\n"
            f"RESTORED_FROM_CACHE: {restored_from_cache}\n"
            f"STDOUT:\n{result.stdout}\n"
            f"STDERR:\n{result.stderr}"
        )
    _refresh_cached_lake(lean_root)
