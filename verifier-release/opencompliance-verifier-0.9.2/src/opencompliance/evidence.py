...
from .evidence_validation import validate_claim
...
class EvidenceRegistry:
    ...
    def from_path(cls, path: Path) -> "EvidenceRegistry":
        ...
        for raw in raw_claims:
            ...
            if reasons:
                raise ValueError(
                    f"invalid evidence claim {raw.get('claimId')} in {path}: "
            ...
            claims.append(
                EvidenceClaim(
                    ...
                    actor_role=raw["actor"].get("actorRole"),
                    signer_type=raw.get("signer", {}).get("signerType"),
                    signer_role=raw.get("signer", {}).get("signerRole"),
