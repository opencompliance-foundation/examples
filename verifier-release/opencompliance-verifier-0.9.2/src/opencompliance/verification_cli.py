...
OUTCOME_ARTIFACTS = ("certificate.json", "punch-list.json")
...
def execute_verify_cli(
    ...
    *,
    ...
    if write:
        generated_outcome = next((name for name in OUTCOME_ARTIFACTS if name in generated), None)
        if generated_outcome is not None:
            for stale_name in OUTCOME_ARTIFACTS:
                if stale_name == generated_outcome:
                    continue
